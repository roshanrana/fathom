...
- Published 2026-09-12 (owner instruction): repository made PUBLIC at github.com/roshanrana/fathom; description and 12 topics set (matching sibling repos); social card `docs/assets/social-card.png` committed; wiki disabled; no GitHub releases (siblings have none). Declined by the owner: profile README rows, LinkedIn entry and post, resume entry — not pending, declined. GitHub's social-preview image cannot be set via API; set it from repo Settings if wanted.
- Deliverables added after ship: `docs/pitch/fathom-architecture-deck.pptx` (+pdf, 27 slides) and live-mode screenshots.
