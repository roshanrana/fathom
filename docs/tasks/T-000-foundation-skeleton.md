...
Attempt 2: Fixed AC6/CI equivalence finding — moved `mcp` (`mcp>=1.9,<2`) and `screenshots` (`playwright>=1.45`) from `[dependency-groups]` to `[project.optional-dependencies]` in pyproject.toml so `uv sync --all-extras` (per AC1 / .github/workflows/check.yml) actually installs them; `dev` remains a dependency group. Updated `tests/test_config.py::test_nfr003_pyproject_declares_lld_dependency_set` to assert `mcp`/`screenshots` live under `project["optional-dependencies"]` instead of `dependency-groups` (the old assertion encoded the bug). Commands run: `uv lock` → resolved 80 packages; `uv sync --all-extras` → installed mcp==1.30.0, playwright==1.62.0 (+11 deps); `uv pip list | grep -Ei "^mcp |^playwright "` → confirms both installed; `uv run python scripts/check.py` → FAILS only on `ruff format --check` for `fathom/providers.py:92` (a concurrent implementer's file, untouched by this attempt — out of scope per instructions); `uv run pytest tests/test_config.py tests/test_errors.py tests/test_data.py tests/test_app_skeleton.py -q` → 25 passed; `uv run ruff check fathom/config.py fathom/errors.py fathom/data.py app scripts` → all checks passed, clean.
